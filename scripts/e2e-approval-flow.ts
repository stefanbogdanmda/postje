...
async function main() {
  ...
  const otherClientId = await seedTestClient(db, "other-client")
  const photoId = await seedTestPhoto(db, clientId, "photo-001")
  ...
  const allPosts = await getPostsByDateRange(db, clientId, "2026-05-12", "2026-05-18")
  ...
  const approved = await approvePost(db, postToApprove.id, clientId)
  ...
  const edited = await approvePost(db, postToEdit.id, clientId, "Edited FB content")
  ...
  const rejected = await rejectPost(db, postToReject.id, clientId)
  ...
  const regenerated = await regeneratePost(db, postToRegen.id, clientId, "New FB Tuesday", "regenerated reasoning")
  ...
  for (let i = 0; i < MAX_REJECTIONS; i++) {
    await regeneratePost(db, postForMaxTest.id, clientId, `Version ${i + 2}`, "regen")
  }
  const maxedPost = (await getPostById(db, postForMaxTest.id, clientId))!
  assert(maxedPost.rejectionCount === MAX_REJECTIONS, `rejectionCount should be ${MAX_REJECTIONS}`)

  // ── Test 7: Client isolation ──
  console.log("\nTest 7: Client isolation")
  const crossClientPost = await getPostById(db, postToApprove.id, otherClientId)
  assert(crossClientPost === null, "Should not find post with wrong clientId")

  let crossClientError = false
  try {
    await approvePost(db, allPosts[3].id, otherClientId)
  } catch {
  ...
  try {
    await approvePost(db, postToApprove.id, clientId) // already approved
  } catch {
